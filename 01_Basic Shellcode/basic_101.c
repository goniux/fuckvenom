...
// forcing strings to be stored in .text section:

#pragma code_seg(".text")

__declspec(allocate(".text")) wchar_t kernel32_str[]=L"kernel32.dll";

__declspec(allocate(".text")) char load_lib_str[]="LoadLibraryA";

//defining main function

int main(){
    // stack based strings for library and funcitions:
    ...
    //resolving kernel32 image base
    LPVOID base = get_module_by_name((const LPWSTR)kernel32_dll_name);
    if (!base){
        return 1;
    }

    //resolving loadlibraryA() address
    LPVOID load_lib=get_func_by_name((HMODULE)base,(LPSTR)load_lib_name);
    if (!load_lib){
        return 2;
    }

    //resolving getprocaddress() address
    ...
    // load user32.dll
    HMODULE user32_dll = _LoadLibraryA((LPCSTR)user32_dll_name);
    if (!user32_dll){
        return 5;
    }
